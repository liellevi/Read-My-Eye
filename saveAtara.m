...
close all;
%0-reg, 1-close, 2-far
syntheticDir   = fullfile(toolboxdir('vision'), 'visiondata','atara1');
trainingSet = imageDatastore(syntheticDir,   'IncludeSubfolders', true, 'LabelSource', 'foldernames');
numImages = numel(trainingSet.Files);
trainingFeatures=zeros(numImages);
for i = 1:numImages
    image1 = readimage(trainingSet, i);
    F=FeaturStatisticalAtara(image1);%compare with database
    label=double(trainingSet.Labels(i))-1;
    F=[F label];
try load Atara1.mat
    F=[F];
    Atara1= [Atara1;F];
    save Atara1.mat Atara1
catch
    Atara1=[F]; % 10 12%
  %db= [db;F];
    save Atara1.mat Atara1
...
function [F]=FeaturStatisticalAtara(im)
...
F=[meanGrayImage sum(sum(BW)) len(1) len(2)];
end