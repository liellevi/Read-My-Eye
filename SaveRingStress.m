...
close all;
%0-NO, 1-YES
syntheticDir   = fullfile(toolboxdir('vision'), 'visiondata','Stress');
trainingSet = imageDatastore(syntheticDir,   'IncludeSubfolders', true, 'LabelSource', 'foldernames');
numImages = numel(trainingSet.Files);
trainingFeatures=zeros(numImages);
for i = 1:numImages
    image1 = readimage(trainingSet, i);
    F=FeaturStatisticalStress(image1);%compare with database
    label=double(trainingSet.Labels(i))-1;
    F(6)=label;
try load StressRing.mat
    F=[F];
    StressRing= [StressRing;F];
    save StressRing.mat StressRing
catch
    StressRing=[F]; % 10 12%
  %db= [db;F];
    save StressRing.mat StressRing
...
function [F]=FeaturStatisticalStress(im)
