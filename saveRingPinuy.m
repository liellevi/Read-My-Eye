...
close all;
%0-NO, 1-YES
syntheticDir   = fullfile(toolboxdir('vision'), 'visiondata','pinuy');
trainingSet = imageDatastore(syntheticDir,   'IncludeSubfolders', true, 'LabelSource', 'foldernames');
numImages = numel(trainingSet.Files);
trainingFeatures=zeros(numImages);
for i = 1:numImages
    image1 = readimage(trainingSet, i);
    F=FeaturStatisticalPinuyRing(image1);%compare with database
try load Pinuy.mat
F=[F];
    Pinuy= [Pinuy;F];
    save Pinuy.mat Pinuy
    catch
  Pinuy=[F];
...
function [F]=FeaturStatisticalPinuyRing(im)
...
maskB = uint8((xx.^2 + yy.^2)<(ci(3)/1.2)^2);
croppedForAverage = uint8(zeros(size(im2Foraverage)));
...
maskBForAverage = uint8((xx.^2 + yy.^2)>(ci(3)/1.7)^2);
...
mask3 = uint8((xx.^2 + yy.^2)>(ci(3)/1.2)^2);
 ...
 for d=1:depth
    for r=1:row 
        for c=1:column
           ...
           if ( RGBsum(r,c) <= averagSumRGB && RGBsum(r,c)~=0)
               NewImage(r,c,d)=0;
...
F=[White_pix/total Black_pix/total meanrB meangB meanbB];
end